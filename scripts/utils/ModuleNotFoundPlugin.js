...
class ModuleNotFoundPlugin {
	...
	prettierError(err) {
		...
		if (request) {
			const isModule = details[1]?.includes("module")
			const isFile = details[1]?.includes("file")

			let [, target, context] = request
			context = this.getRelativePath(context)
			if (isModule) {
				details = [`Cannot find module: '${target}'. Make sure this package is installed.`, "", `You can install this package by running: ${chalk.bold(`npm install ${target}`)}.`]
			} else if (isFile) {
			...
			nmf.hooks.afterResolve.intercept({
				register(tap) {
					...
					return Object.assign({}, tap, {
						fn: (compilation, callback) => {
							tap.fn(compilation, (err, ...args) => {
								if (err?.message?.includes("CaseSensitivePathsPlugin")) {
									err = prettierError(err)
								}
								callback(err, ...args)
							})
